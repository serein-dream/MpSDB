...
def launch_aggregate_server(host, port):
    ...
    if args.model.upper() == "cnn".upper():
    ...
    elif args.model.upper() == "LR".upper():
        model = Logistic(n_f=args.n_features)
    elif args.model.upper() == "Linear".upper():
        model = Linear(n_f=args.n_features)
    elif args.model.upper() == "mlp".upper():
        
        model = MLP(dim_in=args.n_features, dim_hidden=50,
        ...
        #                        dim_out=args.num_classes)
    elif args.model.upper() == "kmeans".upper():
        model = Logistic(n_f=args.n_features)#没有实际意义，只是为了45行接口统一
    ...
    tenseal_aggregate_server_pb2_grpc.add_AggregateServerServiceServicer_to_server(
        AggregateServer(args.num_users, pk_ctx_file, model),
        server)
    server.add_insecure_port(aggr_server_address)
    print(aggr_server_address)
    server.start()
