    ...
    def do_federated_round(self, clients_in_round, centroids):
        ...
        for client_data in clients_in_round:
            ...
            for e in range(self.epochs):
                ...
                #对client_counts中的0变成1，不能除以0
                interim_updates = client_updates_sum / np.expand_dims(np.maximum(client_counts, np.ones_like(client_counts)), axis=1)

                print(f"\n\nnp.ones_like(client_counts):  {np.ones_like(client_counts)} \nnp.expand_dims(np.maximum(client_counts, np.ones_like(client_counts)), axis=1):  {np.expand_dims(np.maximum(client_counts, np.ones_like(client_counts)), axis=1)} \n\n")
                if self.epoch_lr is not None:
            ...
            counts += client_counts
            print(f"counts:  num:{num}     counts:{counts}\n")
            num = num+1
            if self.verbose:
